...
yeahh=random.choice(["YOCHIN","YOCHIN"])
def logo():
	os.system("clear")
	print(f"""          ╔╦╗┌┐ ┌─┐  ╔═╗┌┐ v.1.1
          ║║║├┴┐├┤───╠╣ ├┴┐
...
class ngentod:
		...
		try:
			...
			for softek in memek:
				if "&amp;refid=" in softek[0]:
					self.id.append(re.findall("id=(.*?)&",softek[0])[0]+"[yochin]"+softek[1])
				elif "profile.php?" in softek[0]:
					self.id.append(re.findall("id=(.*)",softek[0])[0]+"[yochin]"+softek[1])
				elif "?refid=" in softek[0]:
					self.id.append(re.findall("(.*?)\?refid=",softek[0])[0]+"[yochin]"+softek[1])
				else:
					self.id.append(softek[0]+"[yochin]"+softek[1])
	...
	def babaturan(self,hencet):
		try:
			...
			for softek in memek:
				if "?uid" in softek[0]:
					self.id.append(re.findall("uid\=(.*?)\&",softek[0])[0]+"[yochin]"+softek[1])
				else:
					self.id.append(re.findall("\/(.*?)\?fref",softek[0])[0]+"[yochin]"+softek[1])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
			if "Lihat selengkapnya" in kontol:
		...
		try:
			...
			for softek in memek:
				if "profile.php?" in softek[1]:
					self.id.append(re.findall("id=(.*?)&amp;refid",softek[1])[0]+"[yochin]"+softek[2])
				else:
					self.id.append(re.findall("(.*?)\?refid=",softek[1])[0]+"[yochin]"+softek[2])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if len(self.id)==jum:
					true=True
					break
			...
			if "Semua 0" in kontol:
				...
				for softek in memek:
					if "profile.php?" in softek[0]:
						...
						self.id.append(re.findall("/(.*)",softek[0])[0]+"[yochin]"+softek[1])
					print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if "Lihat Selengkapnya" in kontol:
				...
				if "profile.php?" in softek[0]:
					...
					if "/" in tol[-1:]:
						...
						if tol in self.id:
							...
							self.id.append(tol+"[yochin]"+softek[1])
				else:
	...
	def fuck(self):
		...
		if pilih in["l","L"]:
			...
			print("[*] Silahkan Hubungi WhatsApp Saya 083175817046");os.system("xdg-open http://wa.me/+6283175817046?text=assalamualaikum");input("[*] Enter Untuk Kembali Ke Menu > ");waktu(2);self.menu()
		elif pilih in["u","U"]:
			os.system("git pull")
			exit()
		elif pilih in["h","H"]:
